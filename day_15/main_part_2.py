...
class Box:
    ...
    def focusing_power(self):
        ...
        for i, lens in enumerate(self.lens_list):
            sum += (1 + self.id) * (i + 1) * int(lens.focal_length)
    ...
    def add_lens(self, lens):
        ...
        lens_found = False
        for lens_candidates in self.lens_list:
            if lens_candidates.label == label:
                lens_candidates.focal_length = focal_length
                lens_found = True
                break
